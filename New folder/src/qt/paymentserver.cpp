...
const QString BITCOIN_IPC_PREFIX("o2ocoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click o2ocoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start o2ocoin: click-to-pay handler");
    else
...
{
    // clicking on o2ocoin: URLs creates FileOpen events on the Mac:
