...
{
    return QCoreApplication::translate("o2ocoin-core", psz).toStdString();
}
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. 020londoncoin can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        ...
        // (which not yet possible because lang=XX can be overridden in o2ocoin.conf in the data directory)
        QMessageBox::critical(0, "020londoncoin",
    ...
    QApplication::setOrganizationName("020londoncoin");
    QApplication::setOrganizationDomain("o2ocoin.org");
    if(GetBoolArg("-testnet")) // Separate UI settings for testnet
        QApplication::setApplicationName("020londoncoin-Qt-testnet");
    else
        QApplication::setApplicationName("020londoncoin-Qt");
    ...
    try
    {
        ...
        if(AppInit2(threadGroup))
        {
            ...
            // Shutdown the core and its threads, but don't exit 020londoncoin-Qt here
